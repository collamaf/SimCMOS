...
//
// $Id: SteppingVerbose.cc 98257 2016-07-04 17:39:46Z gcosmo $
// 
...
//	if(evt->GetEventID()==454741 || evt->GetEventID()==476533 || evt->GetEventID()==547138){
		...
		if( verboseLevel > 0 ){
			...
			<< std::setw(4) << G4BestUnit(fTrack->GetGlobalTime(),"Time")
			<< std::setw(4) << G4BestUnit(fTrack->GetLocalTime(),"Time")
...
//	if(evt->GetEventID()==454741 || evt->GetEventID()==476533 || evt->GetEventID()==547138){
//		G4cout<<"CAPISCO! Sono in StepInfo!"<<G4endl;
		CopyState();
		...
		if( verboseLevel >= 1 ){
			...
			<< std::setw(6) << G4BestUnit(fTrack->GetKineticEnergy(),"Energy")
			<< std::setw(6) << G4BestUnit(fStep->GetTotalEnergyDeposit(),"Energy")
			<< std::setw(4) << G4BestUnit(fStep->GetStepLength(),"Length")
			<< std::setw(4) << G4BestUnit(fTrack->GetTrackLength(),"Length")
			<< std::setw(4) << G4BestUnit(fTrack->GetGlobalTime(),"Time")
			<< std::setw(4) << G4BestUnit(fTrack->GetLocalTime(),"Time")
			<< std::setw(10) << fTrack->GetVolume()->GetName();
			...
			if (verboseLevel == 2) {
				...
				if (nbtrk) {
					...
					for (size_t lp=0; lp<(*secondary).size(); lp++) {
						...
						<< "  Loctime ="
						<< std::setw(6)
						<< G4BestUnit((*secondary)[lp]->GetLocalTime(),"Time");
						G4cout << G4endl;
					}
